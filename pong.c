 ...
 * Optimized: Only redraws moving objects
 */
...
typedef struct {
    int x, y;
    int vx, vy;
    int old_x, old_y;  /* Previous position for erasing */
} ball_t;

typedef struct {
    int y;
    int old_y;  /* Previous position for erasing */
} paddle_t;
...
void pong_init(void) {
    ...
    left_paddle.old_y = left_paddle.y;
    right_paddle.y = PONG_HEIGHT / 2 - PADDLE_HEIGHT / 2;
    right_paddle.old_y = right_paddle.y;
    left_score = 0;
    right_score = 0;
    pong_running = 1;
    needs_full_draw = 1;
...
void pong_draw(void) {
    if (needs_full_draw) {
        /* Full redraw only on init/score */
        ...
        /* Erase old positions */
        vga_fillrect(ball.old_x, ball.old_y, BALL_SIZE, BALL_SIZE, COLOR_BLACK);
        
        if (left_paddle.old_y != left_paddle.y) {
            vga_fillrect(5, left_paddle.old_y, PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_BLACK);
        }
        if (right_paddle.old_y != right_paddle.y) {
            vga_fillrect(PONG_WIDTH - 5 - PADDLE_WIDTH, right_paddle.old_y, PADDLE_WIDTH, PADDLE_HEIGHT, COLOR_BLACK);
        }
    }
